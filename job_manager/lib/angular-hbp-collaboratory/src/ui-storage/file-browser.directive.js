...
function clbFileBrowser(lodash) {
  return {
    ...
    templateUrl: UI_STORAGE_TEMPLATE_DIR+'file-browser.directive.html',
  ...
  function FileBrowserViewModel(
    ...
    vm.browseToEntity = browseToEntityEvent;
    ...
    $rootScope.ctx = $location.search().ctx;/*J.D.*/
    ...
    /**
     * @method browseToEntity
     * @desc
     * When the user make a double click on a file,
     *
     * The event signature is (event, newEntity, previousEntity).
     *
     * @param  {Object} entity selected entity
      ...
      if(file_ext == "ipynb"){
        ...
        .then(
          function succes(response){
            tab_cells = JSON.parse(response.data).cells
            var t_response = [];
            angular.forEach(tab_cells, function(value, key){
        ...
        create_job.code.value = fileUrl +"/"+ file._uuid;
      }
      create_job.hardware_config.value = "{}";
      UiStorageGetData.job.code = create_job.code.value;
      UiStorageGetData.job.hardware_config = create_job.hardware_config.value;
    }
