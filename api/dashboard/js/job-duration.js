d3.json("/statistics/job-duration?scale=log&bins=50", function (error, data) {
  if (error) return console.warn(error);
  var job_durations = {
    spinnaker_finished: data.filter(function (item) {
      return item.platform == "SpiNNaker" && item.status == "finished";
    })[0],
    brainscales_finished: data.filter(function (item) {
      return item.platform == "BrainScaleS" && item.status == "finished";
    })[0],
    brainscales2_finished: data.filter(function (item) {
      return item.platform == "BrainScaleS-2" && item.status == "finished";
    })[0],
    spikey_finished: data.filter(function (item) {
  ...
  job_durations.brainscales2_finished.values.unshift(
    "BrainScaleS-2 (finished)"
  );
