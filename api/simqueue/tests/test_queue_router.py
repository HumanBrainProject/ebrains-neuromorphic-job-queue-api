...
mock_log = {"job_id":999999,"content":"Running from /tmp/job3395001697060736013.tmp; changing to spinnaker"}
mock_job_patch = {
  "status": "submitted",
  "output_data": [
    {
...
def test_get_tags(mocker):
    ...
    response = client.get("/jobs/999999/tags",
                          headers={"Authorization": "Bearer notarealtoken"})
    assert response.status_code == 200
...
def test_get_comments(mocker):
    ...
    response = client.get("/jobs/999999/comments",
                          headers={"Authorization": "Bearer notarealtoken"})
    assert response.status_code == 200
...
def test_get_log(mocker):
    ...
    response = client.get("/jobs/999999/log",
                          headers={"Authorization": "Bearer notarealtoken"})
    assert response.status_code == 200
    
def test_post_job(mocker):
    mocker.patch("simqueue.oauth.User", MockUser)
    mocker.patch("simqueue.db.post_job", return_value=mock_accepted_job)
    response = client.post("/jobs/",
                           json=mock_submitted_job,
                          headers={"Authorization": "Bearer notarealtoken"})
...
def test_put_job(mocker):
    ...
    response = client.put("/jobs/999999",
                           json=mock_job_patch,
                          headers={"Authorization": "Bearer notarealtoken"})
    assert response.status_code == 200