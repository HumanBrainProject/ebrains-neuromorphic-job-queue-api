...
class QueueResource(BaseJobResource):
    ...
    def copy_code_from_collab_drive(self, bundle):
        ...
        collab_name = bundle.data.get('collab_id', None)
        code_location = bundle.data.get("code")
        local_dir = settings.TMP_FILE_ROOT
        get_temporary_url = lambda file_name: bundle.request.build_absolute_uri(settings.TMP_FILE_URL + file_name)
        return copy_code_from_collab_drive(ebrains_drive_client, collab_name, code_location, 
                                           local_dir, get_temporary_url)
